...
def run_dream(parameters, likelihood, nchains=5, niterations=50000, start=None, restart=False, verbose=True, nverbose=10, tempering=False, par_dump=None, **kwargs):
    ...
    if tempering:        
        ...
        if type(start) is list:
            args = zip([step_instance]*nchains, [niterations]*nchains, start, [verbose]*nchains, [nverbose]*nchains, [par_dump]*nchains, range(nchains))

        else:
            args = list(zip([step_instance]*nchains, [niterations]*nchains, [start]*nchains, [verbose]*nchains, [nverbose]*nchains, [par_dump]*nchains, range(nchains)))
