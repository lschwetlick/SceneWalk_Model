...
### ----------------------------------------------------------------
def get_total_list_LL_trials(sw_model, x_dat, y_dat, dur_dat, im_dat, densities_dat):
    """
    Gives a list containing a LL for each scanpath
    Inputs:
        - sw_model: scenewalk model object
        - x_dat, y_dat, dur_dat, im_dat: data of shape trial[path[]] either of whole dataset or of subset (trials running in one pool)
    ...
    for tr in range(len(x_dat)):
        im = im_dat[tr][0]
        path_likelihood = sw_model.get_scanpath_likelihood(x_dat[tr], y_dat[tr], dur_dat[tr], densities_dat[im - 1])
...
def do_multiprocess_trials(function_args, num_processes):
    ...
    if num_processes > 1:
        ...
        results_list = [get_total_list_LL_trials(*some_args) for some_args in function_args]
...
def get_neg_tot_like_trials_parallel(sw_model, x_dat, y_dat, dur_dat, im_dat, densities_dat, num_processes):
    """
    ...
    for input_indices in inputs:
        args_multi.append((sw_model, x_dat[input_indices], y_dat[input_indices], dur_dat[input_indices], im_dat[input_indices], densities_dat))
...
def get_total_neg_LL_per_subj(sw_model, x_dat_sub, y_dat_sub, dur_dat_sub, im_dat_sub, densities_dat):
    """
    Iterate over all images one subject saw and returns the negative Log Likelihood given parameters. (Linear)
    Inputs:
        - sw_model: scenewalk model object
        - x_dat_sub, y_dat_sub, dur_dat_sub, im_dat_sub: data for one subject of shape trial[path[]]
        - densities_dat: empirical densities
    Returns:
    ...
    for tr in range(len(x_dat_sub)):
        im = im_dat_sub[tr][0]
        path_likelihood = sw_model.get_scanpath_likelihood(x_dat_sub[tr], y_dat_sub[tr], dur_dat_sub[tr], densities_dat[im-1])
...
# Likelihood Funtion
def get_total_neg_LL_subjs(sw_model, x_dat, y_dat, dur_dat, im_dat, densities_dat, num_processes_trials):
    """
    Calls get_total_neg_LL_per_subj() for every subject and returns the sum
    Inputs:
        - sw_model: scenewalk model object
        - x_dat, y_dat, dur_dat, im_dat: data of shape subject[trial[path[]]] either of whole dataset or of subset (subjs running in one pool)
        - densities_dat: empirical densities
    Returns:
    ...
    if num_processes_trials > 1:
        for sub in range(len(x_dat)):
            sub_LL_list.append(get_neg_tot_like_trials_parallel(sw_model, x_dat[sub], y_dat[sub], dur_dat[sub], im_dat[sub], densities_dat, num_processes_trials))
    else:
        for sub in range(len(x_dat)):
            sub_LL_list.append(get_total_neg_LL_per_subj(sw_model, x_dat[sub], y_dat[sub], dur_dat[sub], im_dat[sub], densities_dat))
    total_neg_LL = np.sum(sub_LL_list)
    return total_neg_LL
...
def do_multiprocess_subjs(function_args, num_processes):
    ...
    if num_processes > 1:
        ...
        results_list = [get_total_neg_LL_subjs(*some_args) for some_args in function_args]
    return sum(results_list)


def get_neg_tot_like_parallel(sw_model, x_dat, y_dat, dur_dat, im_dat, densities_dat, num_processes_subjs, num_processes_trials=1):
    """
    Scenewalk evaluation parallelized by subject. This is what you call fromt the outside giving it a list of people
    Inputs:
        - sw_model: scenewalk model object
        - x_dat, y_dat, dur_dat, im_dat: data of shape subject[trial[path[]]]
        - densities_dat: empirical densities
    ...
    inputs = [subject_indices[i::num_processes_subjs] for i in range(num_processes_subjs)]
    for input_indices in inputs:
        args_multi.append((sw_model, x_dat[input_indices], y_dat[input_indices], dur_dat[input_indices], im_dat[input_indices], densities_dat, num_processes_trials))
