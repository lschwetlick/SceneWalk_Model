"""
Sample data
Just a simulated data set to see if we can re-estimate the parameters back.

options
- new params for each subject
- sampled params per trial
- simulated or empirical durations
"""
import os
import time
import numpy as np
...
def simulate(dur_dat, im_dat, densities_dat, sw_model, params=None, start_loc="center", x_path=None, y_path=None, resample_durs=False):
    """
    simulates dataset given durations and images
    Inputs:
        - dur_dat : duration vector Subjects[Images[Scanpath[]]]
        - im_dat : image numbers vector (starts at 1)
        - densities_dat : 128x128px densities in the order of number reference
        - sw_model : scenewalk object (if using only one param combination for the whole simulation, pass object with preset param values)
        - params : if None, just use whatever is in the scenewalk object. Otherwise pass a dictionary
        - start_loc  ["center" or "data"]: start location of each trial
        - x_path, y_path : if start_loc is set to data, provide x and y positions of the data
        - resample_durs : set to true if you want durations to be sampled from a fitted gamma distribution for each subject. If False, it uses empirical fixation durations.
    """
    ...
    for sub_dur_dat, sub_im_dat in zip(dur_dat, im_dat): # iterate subjects
        ...
        if resample_durs:
            sub_durs_flat = [item for sublist in sub_dur_dat for item in sublist]
            gamma = stats.gamma
        ...
        for durations, tr_im in zip(sub_dur_dat, sub_im_dat): # iterate trials
            ...
            start_loc_x, start_loc_y = _pick_start_position(start_loc, sw_model, x_path, y_path, sub_cnt, tr_cnt)
...
def simulate_sample(dur_dat, im_dat, densities_dat, sw_model, chains_dict, sample_level, start_loc="center", x_path=None, y_path=None, resample_durs=False):
    """
    simulates dataset given durations and images
    Inputs:
        - dur_dat : duration vector Subjects[Images[Scanpath[]]]
...
def _save_sims(data_list_x, data_list_y, data_list_dur, data_list_im, sw_model, nvp, sampled=False):
    ...
    np.save(folderPath + "/%s_sim_range.npy" % sim_id, [sw_model.data_range['x'], sw_model.data_range['y']])
    meta_dict = {
